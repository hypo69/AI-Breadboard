## \file skills/google-workspace/scripts/gmail_manager.py
# -*- coding: utf-8 -*-
...
try:
    ...
    HttpError = Exception  # Fallback if import fails
...
class GmailManager:
    ...
    def search_messages(self, query: str = "is:unread", max_results: int = 10) -> List[Dict[str, Any]]:
        ...
        if not self.service:
            logger.warning("Gmail service is not initialized (no valid credentials).")
            return []
        ...
        try:
            ...
            if getattr(http_err, "resp", None) and http_err.resp.status == 403:
                logger.warning(
                    f"[GmailManager] Gmail API is not enabled or disabled for this project (HTTP 403). "
                    f"Enable it in Google Cloud Console if needed. Error: {http_err.reason if hasattr(http_err, 'reason') else http_err}"
                )
            else:
                logger.warning(f"[GmailManager] Gmail API request failed with HTTP error: {http_err}")
            return []
        except Exception as e:
            logger.warning(f"[GmailManager] Failed to search Gmail messages: {e}")
            return []
