...
for p in [_current] + list(_current.parents):
...
class RAGDocumentCleaner:
    def __init__(self, min_chunk_len: int = 10, max_chunk_len: int = 1500):
...
def main():
    parser = argparse.ArgumentParser(description="RAG Document Cleaner")
    parser.add_argument("--input", "-i", required=True, help="Input file or directory")
    parser.add_argument("--output", "-o", help="Output JSONL path")
    args = parser.parse_args()

    cleaner = RAGDocumentCleaner()
    chunks = cleaner.process_target(args.input)
