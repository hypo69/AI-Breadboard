...
if ($Help) {
    ...
    Write-Host "║       Run-CloudflaredMonitor.ps1 — HELP AND PARAMETERS        ║" -ForegroundColor Cyan
    Write-Host "╚═══════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "PURPOSE:" -ForegroundColor Yellow
    Write-Host "  Launch Cloudflare Tunnel Monitor microservice / dashboard."
    Write-Host ""
    Write-Host "SYNTAX:" -ForegroundColor Yellow
    Write-Host "  .\launchers\Run-CloudflaredMonitor.ps1 [-Action start|stop|restart|status] [-Mode server|dashboard] [-NewWindow]"
    Write-Host "  .\launchers\Run-CloudflaredMonitor.ps1 --help"
    Write-Host ""
    exit 0
}
...
function Get-AppProcesses {
    Get-CimInstance Win32_Process | Where-Object {
        $_.CommandLine -and $_.CommandLine -match 'apps\.cloudflared_monitor'
    }
}
...
if ($Action -eq 'status') {
    if ($runningProcs) {
        ...
        Write-Host "✅ Cloudflared Monitor is running (PID: $pids, Port: $Port)" -ForegroundColor Green
    } else {
        Write-Host "❌ Cloudflared Monitor is not running." -ForegroundColor Yellow
    }
    exit 0
}
...
if ($Action -in @('stop', 'restart')) {
    if ($runningProcs) {
        Write-Host "🛑 Stopping Cloudflared Monitor..." -ForegroundColor Yellow
        ...
        Write-Host "ℹ️ Cloudflared Monitor was not running." -ForegroundColor DarkGray
    }
    if ($Action -eq 'stop') {
        exit 0
    }
...
if ($Action -in @('start', 'restart')) {
    ...
    if ($existing) {
        ...
        Write-Host "✅ Cloudflared Monitor is already running (PID: $pids)" -ForegroundColor Green
        exit 0
    }

    $appArgs = "-m apps.cloudflared_monitor --mode $Mode --host $HostAddress --port $Port"

    if ($Foreground) {
        Write-Host ""
        Write-Host "╔═══════════════════════════════════════════════════════════════╗" -ForegroundColor Green
        Write-Host "  ☁️ CLOUDFLARED MONITOR RUNNING (FOREGROUND CONSOLE)           " -ForegroundColor Green
        Write-Host "  Host/Port: ${HostAddress}:${Port}                             " -ForegroundColor DarkGray
        Write-Host "  Python:    $venvPython                                        " -ForegroundColor DarkGray
        Write-Host "╚═══════════════════════════════════════════════════════════════╝" -ForegroundColor Green
        Write-Host ""
        Push-Location $projectRoot
        & $venvPython -m apps.cloudflared_monitor --mode $Mode --host $HostAddress --port $Port
        Pop-Location
        exit $LASTEXITCODE
    }

    if ($NewWindow) {
        Write-Host "    Starting Cloudflared Monitor in a separate console window..." -ForegroundColor Cyan
        ...
        if (-not $thisScript) {
            $thisScript = Join-Path $projectRoot "launchers\Run-CloudflaredMonitor.ps1"
        }

        if ($hasWt) {
            $proc = Start-Process wt.exe -ArgumentList "-d `"$projectRoot`" --title `"Cloudflared Monitor ($Port)`" $shellExe -NoExit -ExecutionPolicy Bypass -File `"$thisScript`" -Mode $Mode -HostAddress $HostAddress -Port $Port -Foreground" -PassThru
        } else {
        ...
        $logOutPath = Join-Path $logsDir "cloudflared_monitor.log"
        $logErrPath = Join-Path $logsDir "cloudflared_monitor_stderr.log"

        Write-Host "    Starting Cloudflared Monitor in background process..." -ForegroundColor Cyan
    ...
    if ($proc) {
        Write-Host ""
        Write-Host "╔═══════════════════════════════════════════════════════════════╗" -ForegroundColor Green
        Write-Host "  ✅ CLOUDFLARED MONITOR STARTED!                                " -ForegroundColor Green
        ...
        Write-Host "[ERROR] Failed to start Cloudflared Monitor process." -ForegroundColor Red
        exit 1
    }
}
