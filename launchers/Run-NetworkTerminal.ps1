...
function Get-AppProcesses {
    Get-CimInstance Win32_Process | Where-Object {
        $_.CommandLine -and $_.CommandLine -match 'apps\.network_terminal'
    }
}
...
if ($Action -in @('start', 'restart')) {
    ...
    $appArgs = "-m apps.network_terminal --mode $Mode --host $HostAddress --port $Port"
    ...
    if ($Foreground) {
        ...
        & $venvPython -m apps.network_terminal --mode $Mode --host $HostAddress --port $Port
        Pop-Location
        exit $LASTEXITCODE
    }
