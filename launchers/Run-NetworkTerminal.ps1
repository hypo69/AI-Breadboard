...
if ($Help) {
    ...
    Write-Host "║       Run-NetworkTerminal.ps1 — HELP AND PARAMETERS           ║" -ForegroundColor Cyan
    Write-Host "╚═══════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "PURPOSE:" -ForegroundColor Yellow
    Write-Host "  Launch Network Analyzer Terminal microservice / dashboard."
    Write-Host ""
    Write-Host "SYNTAX:" -ForegroundColor Yellow
    Write-Host "  .\launchers\Run-NetworkTerminal.ps1 [-Action start|stop|restart|status] [-Mode server|dashboard] [-NewWindow]"
    Write-Host "  .\launchers\Run-NetworkTerminal.ps1 --help"
    Write-Host ""
    exit 0
}
...
function Get-AppProcesses {
    Get-CimInstance Win32_Process | Where-Object {
        $_.CommandLine -and ($_.CommandLine -match 'apps\.windows\.network' -or $_.CommandLine -match 'apps\.network_terminal')
    }
}
...
if ($Action -eq 'status') {
    if ($runningProcs) {
        ...
        Write-Host "✅ Network Analyzer Terminal is running (PID: $pids, Port: $Port)" -ForegroundColor Green
    } else {
        Write-Host "❌ Network Analyzer Terminal is not running." -ForegroundColor Yellow
    }
    exit 0
}
...
if ($Action -in @('stop', 'restart')) {
    if ($runningProcs) {
        Write-Host "🛑 Stopping Network Analyzer Terminal..." -ForegroundColor Yellow
        ...
        Write-Host "ℹ️ Network Analyzer Terminal was not running." -ForegroundColor DarkGray
    }
    if ($Action -eq 'stop') {
        exit 0
    }
...
if ($Action -in @('start', 'restart')) {
    ...
    if ($existing) {
        ...
        Write-Host "✅ Network Analyzer Terminal is already running (PID: $pids)" -ForegroundColor Green
        exit 0
    }

    $appArgs = "-m apps.windows.network --mode $Mode --host $HostAddress --port $Port"

    if ($Foreground) {
        Write-Host ""
        Write-Host "╔═══════════════════════════════════════════════════════════════╗" -ForegroundColor Green
        Write-Host "  🌐 NETWORK TERMINAL RUNNING (FOREGROUND CONSOLE)              " -ForegroundColor Green
        Write-Host "  Host/Port: ${HostAddress}:${Port}                             " -ForegroundColor DarkGray
        Write-Host "  Python:    $venvPython                                        " -ForegroundColor DarkGray
        Write-Host "╚═══════════════════════════════════════════════════════════════╝" -ForegroundColor Green
        Write-Host ""
        Push-Location $projectRoot
        & $venvPython -m apps.windows.network --mode $Mode --host $HostAddress --port $Port
        Pop-Location
        exit $LASTEXITCODE
    }

    if ($NewWindow) {
        Write-Host "    Starting Network Analyzer Terminal in a separate console window..." -ForegroundColor Cyan
        ...
        if (-not $thisScript) {
            $thisScript = Join-Path $projectRoot "launchers\Run-NetworkTerminal.ps1"
        }

        if ($hasWt) {
            $proc = Start-Process wt.exe -ArgumentList "-d `"$projectRoot`" --title `"Network Terminal ($Port)`" $shellExe -NoExit -ExecutionPolicy Bypass -File `"$thisScript`" -Mode $Mode -HostAddress $HostAddress -Port $Port -Foreground" -PassThru
        } else {
        ...
        $logOutPath = Join-Path $logsDir "network_terminal.log"
        $logErrPath = Join-Path $logsDir "network_terminal_stderr.log"

        Write-Host "    Starting Network Analyzer Terminal in background process..." -ForegroundColor Cyan
    ...
    if ($proc) {
        Write-Host ""
        Write-Host "╔═══════════════════════════════════════════════════════════════╗" -ForegroundColor Green
        Write-Host "  ✅ NETWORK ANALYZER TERMINAL STARTED!                          " -ForegroundColor Green
        ...
        Write-Host "[ERROR] Failed to start Network Analyzer Terminal process." -ForegroundColor Red
        exit 1
    }
}
