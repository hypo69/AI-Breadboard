...
if ($Help) {
    ...
    Write-Host "║     Run-EnterpriseKnowledge.ps1 — HELP AND PARAMETERS         ║" -ForegroundColor Cyan
    Write-Host "╚═══════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "PURPOSE:" -ForegroundColor Yellow
    Write-Host "  Launch Enterprise Knowledge Platform microservice / console TUI."
    Write-Host ""
    Write-Host "SYNTAX:" -ForegroundColor Yellow
    Write-Host "  .\launchers\Run-EnterpriseKnowledge.ps1 [-Action start|stop|restart|status] [-Mode server|tui] [-NewWindow]"
    Write-Host "  .\launchers\Run-EnterpriseKnowledge.ps1 --help"
    Write-Host ""
    exit 0
}
...
function Get-AppProcesses {
    Get-CimInstance Win32_Process | Where-Object {
        $_.CommandLine -and $_.CommandLine -match 'apps\.enterprise_knowledge'
    }
}
...
if ($Action -eq 'status') {
    if ($runningProcs) {
        ...
        Write-Host "✅ Enterprise Knowledge Platform is running (PID: $pids, Port: $Port)" -ForegroundColor Green
    } else {
        Write-Host "❌ Enterprise Knowledge Platform is not running." -ForegroundColor Yellow
    }
    exit 0
}
...
if ($Action -in @('stop', 'restart')) {
    if ($runningProcs) {
        Write-Host "🛑 Stopping Enterprise Knowledge Platform..." -ForegroundColor Yellow
        ...
        Write-Host "ℹ️ Enterprise Knowledge Platform was not running." -ForegroundColor DarkGray
    }
    if ($Action -eq 'stop') {
        exit 0
    }
...
if ($Action -in @('start', 'restart')) {
    ...
    if ($existing) {
        ...
        Write-Host "✅ Enterprise Knowledge Platform is already running (PID: $pids)" -ForegroundColor Green
        exit 0
    }

    $appArgs = "-m apps.enterprise_knowledge --mode $Mode --host $HostAddress --port $Port"

    if ($Foreground) {
        Write-Host ""
        Write-Host "╔═══════════════════════════════════════════════════════════════╗" -ForegroundColor Green
        Write-Host "  🏢 ENTERPRISE KNOWLEDGE PLATFORM RUNNING (FOREGROUND)         " -ForegroundColor Green
        Write-Host "  Host/Port: ${HostAddress}:${Port}                             " -ForegroundColor DarkGray
        Write-Host "  Python:    $venvPython                                        " -ForegroundColor DarkGray
        Write-Host "╚═══════════════════════════════════════════════════════════════╝" -ForegroundColor Green
        Write-Host ""
        Push-Location $projectRoot
        & $venvPython -m apps.enterprise_knowledge --mode $Mode --host $HostAddress --port $Port
        Pop-Location
        exit $LASTEXITCODE
    }

    if ($NewWindow) {
        Write-Host "    Starting Enterprise Knowledge Platform in a separate console window..." -ForegroundColor Cyan
        ...
        if (-not $thisScript) {
            $thisScript = Join-Path $projectRoot "launchers\Run-EnterpriseKnowledge.ps1"
        }

        if ($hasWt) {
            $proc = Start-Process wt.exe -ArgumentList "-d `"$projectRoot`" --title `"`"Enterprise Knowledge Platform ($Port)`"`" $shellExe -NoExit -ExecutionPolicy Bypass -File `"$thisScript`" -Mode $Mode -HostAddress $HostAddress -Port $Port -Foreground" -PassThru
        } else {
        ...
        $logOutPath = Join-Path $logsDir "enterprise_knowledge.log"
        $logErrPath = Join-Path $logsDir "enterprise_knowledge_stderr.log"

        Write-Host "    Starting Enterprise Knowledge Platform in background process..." -ForegroundColor Cyan
    ...
    if ($proc) {
        Write-Host ""
        Write-Host "╔═══════════════════════════════════════════════════════════════╗" -ForegroundColor Green
        Write-Host "  ✅ ENTERPRISE KNOWLEDGE PLATFORM STARTED!                     " -ForegroundColor Green
        Write-Host "  Address:  https://${HostAddress}:${Port}/helpdesk             " -ForegroundColor Green
        Write-Host "  Mode:     $Mode                                               " -ForegroundColor DarkGray
        Write-Host "  PID:      $($proc.Id)                                         " -ForegroundColor DarkGray
        Write-Host "╚═══════════════════════════════════════════════════════════════╝" -ForegroundColor Green
        Write-Host ""
    } else {
        Write-Host "[ERROR] Failed to start Enterprise Knowledge Platform process." -ForegroundColor Red
        exit 1
    }
}
