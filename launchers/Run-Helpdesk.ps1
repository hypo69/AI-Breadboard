...
.DESCRIPTION
    Launches apps.helpdesk in background or standalone window with Uvicorn.
...
.EXAMPLE
    ...
    .\launchers\Run-Helpdesk.ps1 -NewWindow
    .\launchers\Run-Helpdesk.ps1 -Action status
    .\launchers\Run-Helpdesk.ps1 -Action stop
#>
...
if ($Help) {
    ...
    Write-Host "║        Run-Helpdesk.ps1 — HELP AND PARAMETERS                 ║" -ForegroundColor Cyan
    Write-Host "╚═══════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "PURPOSE:" -ForegroundColor Yellow
    Write-Host "  Launch Helpdesk Support Desk microservice / console TUI."
    Write-Host ""
    Write-Host "SYNTAX:" -ForegroundColor Yellow
    Write-Host "  .\launchers\Run-Helpdesk.ps1 [-Action start|stop|restart|status] [-Mode server|tui] [-NewWindow]"
    Write-Host "  .\launchers\Run-Helpdesk.ps1 --help"
    Write-Host ""
    exit 0
}
...
function Get-AppProcesses {
    Get-CimInstance Win32_Process | Where-Object {
        $_.CommandLine -and $_.CommandLine -match 'apps\.helpdesk'
    }
}
...
if ($Action -eq 'status') {
    if ($runningProcs) {
        ...
        Write-Host "✅ Helpdesk Support Desk is running (PID: $pids, Port: $Port)" -ForegroundColor Green
    } else {
        Write-Host "❌ Helpdesk Support Desk is not running." -ForegroundColor Yellow
    }
    exit 0
}
...
if ($Action -in @('stop', 'restart')) {
    if ($runningProcs) {
        Write-Host "🛑 Stopping Helpdesk Support Desk..." -ForegroundColor Yellow
        ...
        Write-Host "ℹ️ Helpdesk Support Desk was not running." -ForegroundColor DarkGray
    }
    if ($Action -eq 'stop') {
        exit 0
    }
...
if ($Action -in @('start', 'restart')) {
    ...
    if ($existing) {
        ...
        Write-Host "✅ Helpdesk Support Desk is already running (PID: $pids)" -ForegroundColor Green
        exit 0
    }

    $appArgs = "-m apps.helpdesk --mode $Mode --host $HostAddress --port $Port"

    if ($Foreground) {
        Write-Host ""
        Write-Host "╔═══════════════════════════════════════════════════════════════╗" -ForegroundColor Green
        Write-Host "  🛟 HELPDESK SUPPORT DESK RUNNING (FOREGROUND CONSOLE)         " -ForegroundColor Green
        Write-Host "  Host/Port: ${HostAddress}:${Port}                             " -ForegroundColor DarkGray
        Write-Host "  Python:    $venvPython                                        " -ForegroundColor DarkGray
        Write-Host "╚═══════════════════════════════════════════════════════════════╝" -ForegroundColor Green
        Write-Host ""
        Push-Location $projectRoot
        & $venvPython -m apps.helpdesk --mode $Mode --host $HostAddress --port $Port
        Pop-Location
        exit $LASTEXITCODE
    }

    if ($NewWindow) {
        Write-Host "    Starting Helpdesk Support Desk in a separate console window..." -ForegroundColor Cyan
        ...
        if (-not $thisScript) {
            $thisScript = Join-Path $projectRoot "launchers\Run-Helpdesk.ps1"
        }

        if ($hasWt) {
            $proc = Start-Process wt.exe -ArgumentList "-d `"$projectRoot`" --title `"Helpdesk Support Desk ($Port)`" $shellExe -NoExit -ExecutionPolicy Bypass -File `"$thisScript`" -Mode $Mode -HostAddress $HostAddress -Port $Port -Foreground" -PassThru
        } else {
        ...
        $logOutPath = Join-Path $logsDir "helpdesk.log"
        $logErrPath = Join-Path $logsDir "helpdesk_stderr.log"

        Write-Host "    Starting Helpdesk Support Desk in background process..." -ForegroundColor Cyan
    ...
    if ($proc) {
        Write-Host ""
        Write-Host "╔═══════════════════════════════════════════════════════════════╗" -ForegroundColor Green
        Write-Host "  ✅ HELPDESK SUPPORT DESK STARTED!                             " -ForegroundColor Green
        ...
        Write-Host "[ERROR] Failed to start Helpdesk Support Desk process." -ForegroundColor Red
        exit 1
    }
}
