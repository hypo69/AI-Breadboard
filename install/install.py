...
class Installer:
    ...
    def install_dependencies(self, profile: str = "1") -> bool:
        ...
        Args:
            profile (str): Профиль установки (1-6).
        ...
        requirements_map: Dict[str, List[str]] = {
            ...
            "5": [],
            "6": ["install/req/requirements-light.txt"],
        }

        req_files: List[str] = requirements_map.get(profile, [])

        if not req_files and profile != "6":
            return True
        ...
        try:
            ...
            if profile == "6" and not (self.install_dir / "install/req/requirements-light.txt").exists():
                cmd.extend(["-r", str(self.install_dir / "install/req/requirements-core.txt"),
                            "google-genai", "google-antigravity", "langchain-google-genai"])
    ...
    def verify_environment(self, light_version: bool = False) -> bool:
        ...
        if light_version:
            modules.extend(["google.genai", "google_antigravity"])
            ...
            if light_version:
                ...
                if config_path.exists():
                    try:
                        ...
                        cfg["server"]["use_cloudflared"] = False
                        cfg["server"]["client_url"] = "http://127.0.0.1:8000"
                        cfg.setdefault("ai", {})
                        cfg["ai"]["use_foundry"] = False
                        cfg["ai"]["use_ollama"] = False
                        cfg["ai"]["use_gemini_cli"] = True
                        cfg["ai"]["use_agy"] = True
...
def main() -> int:
    ...
    """
    import argparse
    parser = argparse.ArgumentParser(description="AI Breadboard installer")
    parser.add_argument("--language", "--lang", choices=["ru", "en", "es", "he"], default=None, help="Language")
    parser.add_argument("--install-dir", default=None, help="Installation directory")
    parser.add_argument("--install-light-version", "--light", action="store_true", help="Install lightweight version")
    ...
    try:
        if args.language:
            ...
            lang_enum = select_language()

        installer: Installer = Installer(lang_enum)

        install_dir: str = args.install_dir or input("Installation directory [current]: ").strip() or ""

        if args.install_light_version:
            print(installer.msg("welcome"))
            installer.install_dir = Path(install_dir) if install_dir else Path.cwd()
            installer.venv_dir = installer.install_dir / "venv"
            installer.python_path = installer.venv_dir / ("Scripts/python.exe" if installer.platform_name == "Windows" else "bin/python")

            python_exe = installer.find_python()
            if not python_exe or not installer.create_venv(python_exe) or not installer.upgrade_pip():
                return 1
            if not installer.install_dependencies(profile="6"):
                return 1
            if not installer.verify_environment(light_version=True):
                return 1
            print(installer.msg("finish"))
            return 0
        else:
