...
$Global:I18N = @{
    ru = @{
        ...
        step_8                = "[8/8] Выбор и загрузка моделей для локальных провайдеров..."
        step_8_available      = "    Доступные модели для {0}:"
        step_8_prompt         = "    Выберите модели для загрузки (номера через запятую или диапазон 1-3) [Enter = пропустить]"
        step_8_default_hint   = "    Подсказка: можно выбрать модели по умолчанию: {0}"
        step_8_no_models      = "    [INFO] Нет доступных моделей для провайдера {0}"
        step_8_downloading    = "    Загрузка модели {0} ({1})..."
        step_8_success        = "    [OK] Модель {0} успешно загружена"
        step_8_failed         = "    [ERROR] Не удалось загрузить модель {0}: {1}"
        step_8_skipped        = "    [INFO] Пропущено: {0} ({1})"
        step_8_no_venv        = "    [ERROR] Виртуальное окружение не найдено. Пропуск шага выбора моделей."
        step_8_error          = "    [ERROR] Ошибка при выборе моделей: {0}"
        step_8_skip           = "    [INFO] Шаг выбора моделей пропущен"
        step_8_completed      = "    [OK] Шаг выбора моделей завершен"
        step_8_failed_fetch   = "    [WARN] Не удалось получить список моделей для {0}: {1}"
        step_8_start_download = "    Начало загрузки выбранных моделей..."
        step_8_foundry_hint   = "    [INFO] Foundry загружает модели автоматически при первом использовании"
        step_8_onnx_hint      = "    [INFO] ONNX модели загружаются через Transformers при первом использовании"
        step_8_hf_hint        = "    [INFO] HuggingFace модели загружаются автоматически через Transformers"
        ...
        step_user             = "[9/9] Настройка учетной записи администратора по умолчанию..."
        step_user_header      = "║       НАСТРОЙКА УЧЕТНОЙ ЗАПИСИ АДМИНИСТРАТОРА (ROOT)         ║"
        step_user_prompt_email= "    Email администратора [По умолчанию: admin@localhost]"
        step_user_prompt_name = "    Имя администратора [По умолчанию: Admin]"
        step_user_prompt_pass = "    Пароль администратора [По умолчанию: onela]"
        step_user_ok          = "    [OK] Учетная запись администратора настроена: {0}"
        step_user_err         = "    [WARN] Ошибка при настройке учетной записи администратора: {0}"
    ...
    en = @{
        ...
        step_8                = "[8/8] Selecting and downloading models for local providers..."
        step_8_available      = "    Available models for {0}:"
        step_8_prompt         = "    Select models to download (numbers separated by comma or range 1-3) [Enter = skip]"
        step_8_default_hint   = "    Hint: can select default models: {0}"
        step_8_no_models      = "    [INFO] No available models for provider {0}"
        step_8_downloading    = "    Downloading model {0} ({1})..."
        step_8_success        = "    [OK] Model {0} downloaded successfully"
        step_8_failed         = "    [ERROR] Failed to download model {0}: {1}"
        step_8_skipped        = "    [INFO] Skipped: {0} ({1})"
        step_8_no_venv        = "    [ERROR] Virtual environment not found. Skipping model selection step."
        step_8_error          = "    [ERROR] Error selecting models: {0}"
        step_8_skip           = "    [INFO] Model selection step skipped"
        step_8_completed      = "    [OK] Model selection step completed"
        step_8_failed_fetch   = "    [WARN] Failed to fetch models for {0}: {1}"
        step_8_start_download = "    Starting download of selected models..."
        step_8_foundry_hint   = "    [INFO] Foundry downloads models automatically on first use"
        step_8_onnx_hint      = "    [INFO] ONNX models download via Transformers on first use"
        step_8_hf_hint        = "    [INFO] HuggingFace models download automatically via Transformers"
        ...
        step_user             = "[9/9] Configuring default administrator account..."
        step_user_header      = "║       DEFAULT ADMINISTRATOR ACCOUNT CONFIGURATION (ROOT)      ║"
        step_user_prompt_email= "    Admin email [Default: admin@localhost]"
        step_user_prompt_name = "    Admin name [Default: Admin]"
        step_user_prompt_pass = "    Admin password [Default: onela]"
        step_user_ok          = "    [OK] Administrator account configured: {0}"
        step_user_err         = "    [WARN] Failed to configure administrator account: {0}"
    ...
    es = @{
        ...
        step_8                = "[8/8] Selección y descarga de modelos para proveedores locales..."
        step_8_available      = "    Modelos disponibles para {0}:"
        step_8_prompt         = "    Seleccione modelos para descargar (números separados por coma o rango 1-3) [Enter = omitir]"
        step_8_default_hint   = "    Sugerencia: se pueden seleccionar modelos por defecto: {0}"
        step_8_no_models      = "    [INFO] No hay modelos disponibles para el proveedor {0}"
        step_8_downloading    = "    Descargando modelo {0} ({1})..."
        step_8_success        = "    [OK] Modelo {0} descargado exitosamente"
        step_8_failed         = "    [ERROR] No se pudo descargar el modelo {0}: {1}"
        step_8_skipped        = "    [INFO] Omitido: {0} ({1})"
        step_8_no_venv        = "    [ERROR] Entorno virtual no encontrado. Omitiendo paso de selección de modelos."
        step_8_error          = "    [ERROR] Error al seleccionar modelos: {0}"
        step_8_skip           = "    [INFO] Paso de selección de modelos omitido"
        step_8_completed      = "    [OK] Paso de selección de modelos completado"
        step_8_failed_fetch   = "    [WARN] No se pudo obtener lista de modelos para {0}: {1}"
        step_8_start_download = "    Iniciando descarga de modelos seleccionados..."
        step_8_foundry_hint   = "    [INFO] Foundry descarga modelos automáticamente al primer uso"
        step_8_onnx_hint      = "    [INFO] Los modelos ONNX se descargan vía Transformers al primer uso"
        step_8_hf_hint        = "    [INFO] Los modelos HuggingFace se descargan automáticamente vía Transformers"
        
        step_user             = "[9/9] Configurando cuenta de administrador predeterminada..."
        step_user_header      = "║       CONFIGURACIÓN DE CUENTA DE ADMINISTRADOR (ROOT)        ║"
        step_user_prompt_email= "    Email de administrador [Predeterminado: admin@localhost]"
        step_user_prompt_name = "    Nombre de administrador [Predeterminado: Admin]"
        step_user_prompt_pass = "    Contraseña de administrador [Predeterminado: onela]"
        step_user_ok          = "    [OK] Cuenta de administrador configurada: {0}"
        step_user_err         = "    [WARN] Error al configurar cuenta de administrador: {0}"
    ...
    he = @{
        ...
        step_8                = "[8/8] בחירה והורדת מודלים לספקים מקומיים..."
        step_8_available      = "    מודלים זמינים עבור {0}:"
        step_8_prompt         = "    בחר מודלים להורדה (מספרים מופרדים בפסיק או טווח 1-3) [Enter = דלג]"
        step_8_default_hint   = "    רמז: ניתן לבחור מודלים מוגדרים כברירת מחדל: {0}"
        step_8_no_models      = "    [INFO] אין מודלים זמינים לספק {0}"
        step_8_downloading    = "    מוריד מודל {0} ({1})..."
        step_8_success        = "    [OK] מודל {0} הורד בהצלחה"
        step_8_failed         = "    [ERROR] נכשל בהורדת מודל {0}: {1}"
        step_8_skipped        = "    [INFO] דילג: {0} ({1})"
        step_8_no_venv        = "    [ERROR] סביבה וירטואלית לא נמצאה. דילג על צעד בחירת מודלים."
        step_8_error          = "    [ERROR] שגיאה בבחירת מודלים: {0}"
        step_8_skip           = "    [INFO] צעד בחירת מודלים דולג"
        step_8_completed      = "    [OK] צעד בחירת מודלים הושלם"
        step_8_failed_fetch   = "    [WARN] נכשל בקבלת רשימת מודלים עבור {0}: {1}"
        step_8_start_download = "    מתחיל הורדת מודלים שנבחרו..."
        step_8_foundry_hint   = "    [INFO] Foundry מורד מודלים באופן אוטומטי בשימוש ראשון"
        step_8_onnx_hint      = "    [INFO] מודלים ONNX מתקבלים דרך Transformers בשימוש ראשון"
        step_8_hf_hint        = "    [INFO] מודלים HuggingFace מתקבלים באופן אוטומטי דרך Transformers"
        
        step_user             = "[9/9] הגדרת חשבון מנהל מערכת ברירת מחדל..."
        step_user_header      = "║       הגדרת חשבון מנהל מערכת (ROOT)                          ║"
        step_user_prompt_email= "    דוא\"ל מנהל [ברירת מחדל: admin@localhost]"
        step_user_prompt_name = "    שם מנהל [ברירת מחדל: Admin]"
        step_user_prompt_pass = "    סיסמת מנהל [ברירת מחדל: onela]"
        step_user_ok          = "    [OK] חשבון מנהל המערכת הוגדר בהצלחה: {0}"
        step_user_err         = "    [WARN] שגיאה בהגדרת חשבון מנהל המערכת: {0}"
