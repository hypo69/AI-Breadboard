...
def main() -> None:
    ...
    if args.mode == "server":
        ...
        if server_mode == "shared" and args.port == 0:
            print("[Trading Terminal] App configured in 'shared' server mode.")
            print(f"[Trading Terminal] API endpoints are routed via the main server (http://{effective_host}:8000).")
