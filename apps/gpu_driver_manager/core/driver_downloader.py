...
class DriverDownloader:
    ...
    def get_cached_file(self, filename: str) -> Optional[Path]:
        ...
        if target.exists() and target.stat().st_size > 1024 * 1024:  # больше 1 МБ
            return target
        return None
    ...
    def start_download(
        ...
        # Если файл уже существует и имеет валидный размер, сразу завершаем
        ...
        # Запуск фонового асинхронного потока скачивания
    ...
    async def _download_worker(self, task_id: str, url: str, dest_path: Path) -> None:
        ...
        def _sync_download():
            ...
            try:
                ...
                # Завершение скачивания: атомарное переименование
                if temp_path.exists():
                    if dest_path.exists():
                        dest_path.unlink()
