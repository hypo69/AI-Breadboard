# -*- coding: utf-8 -*-
# =============================================================================
# Process Name: System Log Center CLI Main Entry Point
# =============================================================================
# Description:
#   Точка входа CLI для запуска терминального интерфейса (TUI), вывода логов
#   или запуска автономного FastAPI сервера для Центра системных журналов Windows.
#
# Examples:
#   >>> python -m apps.system_log_viewer --mode server --port 8108
#   >>> python -m apps.system_log_viewer --mode dashboard
#
# File: __main__.py
...
"""CLI точка входа для приложения System Log Center."""
...
from .tui import run_log_dashboard
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="System Log Center - Windows Event Log Viewer & Diagnostic Hub"
    )
    parser.add_argument(
        "--mode",
        "-m",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Порт FastAPI сервера (по умолчанию: 8108)",
    )
    parser.add_argument(
        "--reload",
        action="store_true",
    ...
    if args.mode in ("dashboard", "tui"):
        try:
            asyncio.run(run_log_dashboard(interval=args.interval))
        except KeyboardInterrupt:
            print("\n[System Log Center] Dashboard closed.")
            sys.exit(0)
        return
    ...
    effective_port = args.port if args.port > 0 else getattr(server_cfg, "port", 8108)
...
def _run_standalone_server(
    ...
    """Запуск System Log Center как автономного FastAPI сервиса."""
    try:
        import uvicorn
        from fastapi import FastAPI
        from .router import init_router

        print("[System Log Center] Starting standalone FastAPI server...")
        print(f"  Host: {host}")
        print(f"  Port: {port}")
        ...
        app = FastAPI(
            ...
            description="Windows Event Log Discovery, Stream & Analysis Microservice",
            version="1.0.0",
        )
