...
class UserManager:
    ...
    def delete_user_workspace(self, user_id: int | str, remove_files: bool = True) -> bool:
        ...
        user_dir = self.users_dir / safe_id
        success = True
        if user_dir.exists() and remove_files:
            try:
    ...
    def get_orphaned_user_directories(self) -> List[Dict[str, Any]]:
        """Find orphaned user workspace directories not associated with any active database user.

        Returns:
            List[Dict[str, Any]]: List of orphaned directory metadata including
                name, path, size_bytes, files_count, modified_at, is_test_dir, reason.
        """
    ...
    def cleanup_orphaned_user_directories(self, dir_names: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        Args:
            dir_names (Optional[List[str]]): Specific directory names to clean. If None, cleans all.
