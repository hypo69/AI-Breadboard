...
async def ifttt_trigger_event(
    ...
    и запускать любые настроенные апплеты IFTTT Maker Webhooks.

    Args:
        event_name: Имя события IFTTT (например 'light_on', 'ac_cool', 'movie_mode', 'send_notification').
        value1: Первое опциональное текстовое значение параметра.
        value2: Второе опциональное текстовое значение параметра.
        value3: Третье опциональное текстовое значение параметра.
        json_data: Дополнительные параметры в формате JSON-строки (для сложных апплетов).
    """
    try:
        from plugins.ifttt import IFTTTClient
        client = IFTTTClient()
        payload = {}
        if json_data:
            try:
