...
class UserWorkspaceRAGManager:
    ...
    def sync_google_docs(
        ...
        user_id: int,
        rag_id: str,
        query: str = "mimeType = 'application/vnd.google-apps.document' and trashed = false",
        account_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """Download Google Docs via GDriveManager and index them into the user's RAG collection.

        Args:
            user_id (int): User unique identifier.
            rag_id (str): Collection identifier.
            query (str): Google Drive search query.
            account_name (Optional[str]): Optional Google Workspace account name.
        ...
        try:
            ...
            raise RuntimeError("GDriveManager is not available in google-workspace skill.")

        manager = GDriveManager(account_name=account_name or None)
        if not manager.service:
            raise RuntimeError(f"Google Workspace authentication failed for account '{account_name or 'default'}'.")

        user_files_dir = user_manager.get_user_directory(user_id, subfolder="files", create=True)
        gdocs_dir = user_files_dir / "google_docs"
        gdocs_dir.mkdir(parents=True, exist_ok=True)

        files = manager.list_files(query=query if query else None, page_size=50)
        ...
        for f in files:
            ...
            if success:
                # Find matching target file with extension (e.g. .docx)
                stem = Path(name).stem
                targets = list(gdocs_dir.glob(f"{stem}*"))
                for tf in targets:
                    rel_path = str(tf.relative_to(user_files_dir)).replace("\\", "/")
                    if rel_path not in downloaded_names:
                        downloaded_names.append(rel_path)

        # Now trigger collection build with the downloaded files
        build_res = self.build_collection(
            user_id=user_id,
            rag_id=rag_id,
        ...
        return {
            ...
            "chunks_count": build_res.get("chunks_count", 0),
            "collection": build_res.get("collection")
        }
