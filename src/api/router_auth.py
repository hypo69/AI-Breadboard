...
@router.post('/logout')
async def logout_post(response: Response) -> dict:
    """Выход пользователя из системы через POST API.
...
async def logout_get() -> RedirectResponse:
    """Выход пользователя из системы через GET с перенаправлением на страницу входа.
    
    Returns:
        RedirectResponse: Редирект на корень приложения.
    """
    redir = RedirectResponse(url='/', status_code=303)
    redir.delete_cookie('auth_token', path='/')
    redir.delete_cookie('oauth_state', path='/')
    return redir
