...
window.openPluginFromDropdown = function(pluginName) {
  ...
  if (pluginName === 'telegram_channel_rag' || pluginName === 'telegram_rag') {
  ...
  if (pluginName === 'news_feed' || pluginName === 'news') {
    if (typeof window.switchTab === 'function') {
...
function onTabSwitched(targetId) {
  ...
  if (cleanId === 'tab-chat') {
    ...
    console.log('[AdminInterface] Switching to helpdesk tab...');
...
async function initInterface() {
  ...
  await Promise.all([
    ...
    loadTabContent('helpdesk', `/html/helpdesk_tab/index.html?v=${cb}`, `/html/helpdesk_tab/main.js?v=${cb}`),
