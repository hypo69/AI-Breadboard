...
function saveLocalSessionsOnly() {
  try {
...
function saveSessions(syncToServer = true) {
  saveLocalSessionsOnly();
  if (syncToServer) {
    debounceSyncActiveSession();
  }
}

function debounceSyncActiveSession() {
...
async function syncSessionToServer(session) {
  ...
  try {
    ...
    console.debug('Background session sync to server skipped or failed:', err);
  }
}
...
async function deleteSessionFromServer(sessionId) {
  ...
  try {
    ...
    console.debug('Failed to delete session on server:', err);
  }
}

async function clearAllSessionsFromServer() {
  try {
    await fetch('/api/chat/sessions', {
      method: 'DELETE'
    });
  } catch (err) {
    console.debug('Failed to clear sessions on server:', err);
  }
}
...
async function syncSessionsWithServer() {
  try {
    ...
    // If we have local sessions and server is empty, or server has fewer/older sessions, bulk sync
    if (sessions.length > 0 && serverSessions.length === 0) {
      const syncRes = await fetch('/api/chat/sessions/sync', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      // Merge server sessions with any unique local sessions
      const serverMap = new Map();
      serverSessions.forEach((s) => serverMap.set(s.id, s));
      ...
      sessions.forEach((ls) => {
        if (!serverMap.has(ls.id)) {
          ...
          // If local has newer updatedAt, prefer local and sync to server
          const ss = serverMap.get(ls.id);
          if ((ls.updatedAt || 0) > (ss.updatedAt || 0)) {
      ...
      if (localOnly.length > 0) {
        ...
        await fetch('/api/chat/sessions/sync', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ sessions: localOnly })
        });
      }

      sessions = Array.from(serverMap.values()).sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0));
    }
    ...
    console.debug('Failed to sync sessions with server (working in offline/local mode):', err);
  }
}
...
function initChatTab() {
  ...
  // Expose sendMessage globally for user menu actions
...
function handleRemoteTranscript(text) {
  ...
  // Add message to DOM and save to active session without triggering AI model generation
...
if (typeof window !== 'undefined') {
  ...
  window.handleRemoteTranscript = handleRemoteTranscript;
