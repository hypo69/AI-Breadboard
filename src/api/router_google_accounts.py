...
@router.get("/{name}/drive-files", response_model=Dict[str, Any])
async def list_gdrive_files_for_account(
    name: str,
    query: Optional[str] = Query(None, description="Drive search query"),
    limit: int = Query(20, ge=1, le=100)
) -> Dict[str, Any]:
    """List Google Drive / Docs files for the specified account."""
    info = get_account_info(name)
    if not info:
        raise HTTPException(status_code=404, detail=f"Account '{name}' not found")
    ...
    try:
        ...
        manager = GDriveManager(account_name=name)
        if not manager.service:
            raise HTTPException(status_code=400, detail=f"Cannot initialize Drive service for account '{name}'. Check credentials.")
        files = manager.list_files(query=query if query else None, page_size=limit)
        return {"status": "success", "account": name, "files": files, "count": len(files)}
    except Exception as ex:
        logger.error(f"Error listing Drive files for '{name}': {ex}")
        raise HTTPException(status_code=500, detail=str(ex))
...
async def get_google_sheet_info(name: str, spreadsheet_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        manager = GSheetsManager(account_name=name)
        if not manager.service:
            raise HTTPException(status_code=400, detail=f"Cannot initialize Sheets service for '{name}'")
        info = manager.get_spreadsheet_info(spreadsheet_id)
        if not info:
            raise HTTPException(status_code=404, detail=f"Spreadsheet '{spreadsheet_id}' not found")
        return {"status": "success", "account": name, "spreadsheet": info}
    except HTTPException:
        raise
    except Exception as ex:
        logger.error(f"Error reading spreadsheet info for '{spreadsheet_id}': {ex}")
        raise HTTPException(status_code=500, detail=str(ex))


@router.post("/{name}/sheets/{spreadsheet_id}/read", response_model=Dict[str, Any])
async def read_google_sheet_values(
    name: str,
    spreadsheet_id: str,
    ...
    try:
        ...
        if not manager.service:
            raise HTTPException(status_code=400, detail=f"Cannot initialize Sheets service for '{name}'")
        rows = manager.read_range(spreadsheet_id, range_name=payload.range_name)
        return {"status": "success", "account": name, "range": payload.range_name, "values": rows, "row_count": len(rows)}
    except Exception as ex:
        logger.error(f"Error reading range from '{spreadsheet_id}': {ex}")
        raise HTTPException(status_code=500, detail=str(ex))


@router.post("/{name}/sheets/{spreadsheet_id}/append", response_model=Dict[str, Any])
async def append_google_sheet_values(
    name: str,
    spreadsheet_id: str,
    ...
    try:
        ...
        if not manager.service:
            raise HTTPException(status_code=400, detail=f"Cannot initialize Sheets service for '{name}'")
        res = manager.append_row(
            spreadsheet_id=spreadsheet_id,
            range_name=payload.range_name,
            values=payload.values,
            value_input_option=payload.value_input_option
        )
        return {"status": "success", "account": name, "result": res}
    except Exception as ex:
        logger.error(f"Error appending row to '{spreadsheet_id}': {ex}")
        raise HTTPException(status_code=500, detail=str(ex))
