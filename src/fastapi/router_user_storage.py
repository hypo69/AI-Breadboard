...
class SyncGoogleDocsRequest(BaseModel):
    """Payload for importing and synchronizing Google Docs into a user RAG collection."""
    query: str = Field(default="mimeType = 'application/vnd.google-apps.document' and trashed = false", description="Google Drive query")
    account_name: Optional[str] = Field(default=None, description="Optional Google account name")
...
@user_rags_router.post('/{rag_id}/sync-google-docs')
async def sync_google_docs_user_rag_endpoint(
    request: Request,
    rag_id: str,
    payload: SyncGoogleDocsRequest
) -> Dict[str, Any]:
    """Sync Google Docs / Drive files into the collection and build index.

    Args:
        request: FastAPI request object.
        rag_id: Collection identifier.
        payload: Query and optional account name.

    Returns:
        Dict[str, Any]: Sync summary and updated manifest.
    """
    user_id = _get_current_user_id(request)
    try:
        result = user_workspace_rag_manager.sync_google_docs(
            user_id=user_id,
            rag_id=rag_id,
            query=payload.query,
        ...
        logger.error(f"Error syncing Google Docs for RAG {rag_id}: {ex}")
