...
```powershell
python .agents/skills/gdrive-organizer/scripts/main.py audit
python .agents/skills/gdrive-organizer/scripts/main.py audit --report-out reports/drive_health.md
```

### 2. Generate Reorganization Plan:
```powershell
python .agents/skills/gdrive-organizer/scripts/main.py propose --out plan.json --report-out plan_preview.md
```

### 3. Dry-run Preview of Plan:
```powershell
python .agents/skills/gdrive-organizer/scripts/main.py apply --plan plan.json --dry-run
```

### 4. Execute Restructuring Plan:
```powershell
python .agents/skills/gdrive-organizer/scripts/main.py apply --plan plan.json --log-out execution_log.json
```
