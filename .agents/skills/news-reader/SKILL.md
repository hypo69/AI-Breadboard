...
1. **Сгенерировать дайджест новостей**:
   ```powershell
   python .agents/skills/news-reader/scripts/news_cli.py digest --user-id default
   ```

2. **Получить персонализированный список топ-новостей**:
   ```powershell
   python .agents/skills/news-reader/scripts/news_cli.py feed --limit 5
   ```

3. **Отправить реакцию/обратную связь (обучение)**:
   ```powershell
   python .agents/skills/news-reader/scripts/news_cli.py feedback --article-id "art_id" --action like --title "Заголовок статьи" --category ai
   ```

4. **Просмотреть профиль выученных интересов**:
   ```powershell
   python .agents/skills/news-reader/scripts/news_cli.py profile
   ```
