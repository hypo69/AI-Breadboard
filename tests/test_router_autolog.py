# -*- coding: utf-8 -*-
# =============================================================================
# Process Name: Test Auto-Logging REST API Router
# =============================================================================
# Description:
#   Тесты для эндпоинтов src/api/router_autolog.py: проверка статуса,
...
from src.api.router_autolog import init_router
...
def test_get_autolog_status(test_app: TestClient):
    """Проверяет получение текущего статуса автологгирования."""
    response = test_app.get("/api/autolog/status")
...
def test_get_autolog_config(test_app: TestClient):
    """Проверяет получение текущей конфигурации логгеров."""
    response = test_app.get("/api/autolog/config")
    assert response.status_code == 200
    data = response.json()
    assert data["status"] == "success"
...
def test_poll_all_and_single_logger(test_app: TestClient):
    ...
    # Опрос всех логгеров
    resp_all = test_app.post("/api/autolog/poll-all")
    ...
    resp_single = test_app.post("/api/autolog/poll/system_inspector")
...
def test_list_read_delete_csv_files(test_app: TestClient, tmp_path: Path):
    ...
    # 2. Список файлов
    resp_files = test_app.get("/api/autolog/files")
    ...
    resp_read = test_app.get("/api/autolog/file/test_metric_polls.csv")
    ...
    # 4. Скачивание файла
    resp_down = test_app.get("/api/autolog/download/test_metric_polls.csv")
    assert resp_down.status_code == 200
    assert "25.4" in resp_down.text

    # 5. Удаление файла
    resp_del = test_app.get("/api/autolog/file/test_metric_polls.csv")
    assert resp_del.status_code == 200
    del_res = test_app.delete("/api/autolog/file/test_metric_polls.csv")
    assert del_res.status_code == 200
    assert del_res.json()["status"] == "success"

    # Проверяем, что файл удален
    resp_after = test_app.get("/api/autolog/file/test_metric_polls.csv")
...
async def test_start_and_stop_autolog(test_app: TestClient):
    ...
    resp_start = test_app.post("/api/autolog/start")
