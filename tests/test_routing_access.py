...
def test_root_unauthenticated_serves_login_page(client):
    """Test that root endpoint serves Login / Auth Gate for unauthenticated visitors."""
    response = client.get("/", headers={"host": "kino.davidka.net"})
    assert response.status_code == 200
    html = response.text
    # Auth gate elements must exist
    assert 'id="form-login"' in html
    assert 'id="form-register"' in html
    assert 'id="btn-google-auth"' in html
    # Home dashboard tabs must NOT be present
    assert 'id="tab-chat"' not in html
...
def test_root_authenticated_serves_user_interface(client):
    ...
    assert response.status_code == 200
    html = response.text
    # Check that the tabs exist in User Interface
...
def test_login_endpoint():
    """Test /login serving login page or redirecting when authenticated."""
    from fastapi.testclient import TestClient
    ...
    assert resp_auth.headers["location"] == "/"


def test_user_endpoint_redirects_to_root(client):
    """Test that /user endpoint redirects to root /."""
