...
class TestUserManagerStorage:
    ...
    def custom_user_manager(self, tmp_path):
        """Create an isolated UserManager with temporary DB and users_dir starting with test_."""
        from src.user_manager import UserManager
        db_path = tmp_path / "test_users.db"
        users_dir = tmp_path / "test_user_storage"
    ...
    def test_sqlite_trigger_workspace_cleanup_on_raw_sql_delete(self, custom_user_manager):
        """Verify that SQLite AFTER DELETE trigger automatically removes user workspace on raw SQL DELETE."""
        user_id = custom_user_manager.add_user(
        ...
        with custom_user_manager._get_connection() as conn:
            conn.execute("DELETE FROM users WHERE id = ?", (user_id,))
            conn.commit()

        # Check directory was automatically cleaned up via trigger + UDF
        assert not user_dir.exists()

    def test_orphaned_directories_detection_and_cleanup(self, custom_user_manager):
        """Test detecting orphaned directories and cleaning them up safely."""
        # Active user
        active_id = custom_user_manager.add_user(
            email="active_user@example.com",
            name="Active User"
        )
        active_dir = custom_user_manager.get_user_directory(active_id)
        (active_dir / "files" / "doc.txt").write_text("active doc", encoding="utf-8")

        # Create dead/orphaned directory (simulating deleted user or test leftover)
        dead_dir = custom_user_manager.users_dir / "9998"
        dead_dir.mkdir(parents=True, exist_ok=True)
        (dead_dir / "sample.txt").write_text("orphan data", encoding="utf-8")

        # Create test leftover directory starting with test_
        test_leftover = custom_user_manager.users_dir / "test_old_workspace"
        test_leftover.mkdir(parents=True, exist_ok=True)
        (test_leftover / "temp.log").write_text("test log", encoding="utf-8")

        # Scan for orphaned directories
        orphaned = custom_user_manager.get_orphaned_user_directories()
        orphaned_names = [d["name"] for d in orphaned]
        assert "9998" in orphaned_names
        assert "test_old_workspace" in orphaned_names
        assert str(active_id) not in orphaned_names

        # Cleanup single orphaned directory
        res1 = custom_user_manager.cleanup_orphaned_user_directories(dir_names=["9998"])
        assert res1["total_deleted"] == 1
        assert "9998" in res1["deleted_dirs"]
        assert not dead_dir.exists()
        assert test_leftover.exists()
        assert active_dir.exists()

        # Cleanup all remaining orphaned directories
        res2 = custom_user_manager.cleanup_orphaned_user_directories()
        assert res2["total_deleted"] == 1
        assert "test_old_workspace" in res2["deleted_dirs"]
        assert not test_leftover.exists()
        assert active_dir.exists()
...
class TestFavoriteModels:
    ...
    def user_mgr(self, tmp_path):
        ...
        users_dir = tmp_path / "test_fav_storage"
