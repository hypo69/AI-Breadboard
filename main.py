...
def _build_cors_config() -> dict[str, any]:
    """Build secure CORS configuration parameters from config.json (server.cors / server)."""
    cors_section = getattr(server_cfg, 'cors', None)
    
    # 1. Base / explicitly specified origins from config
    origins: list[str] = [
        "http://localhost",
        "https://localhost",
        "http://127.0.0.1",
    ...
    if cors_section and hasattr(cors_section, 'allow_origins') and isinstance(cors_section.allow_origins, list):
        for o in cors_section.allow_origins:
    ...
    if cors_section and getattr(cors_section, 'allow_origin_regex', None):
        origin_regex = cors_section.allow_origin_regex
    else:
    ...
    # 3. Credentials, methods, headers
    allow_credentials = getattr(cors_section, 'allow_credentials', True) if cors_section else True
    allow_methods = getattr(cors_section, 'allow_methods', ['*']) if cors_section else ['*']
    allow_headers = getattr(cors_section, 'allow_headers', ['*']) if cors_section else ['*']
    allow_methods_list = list(allow_methods) if isinstance(allow_methods, list) else [str(allow_methods)]
    allow_headers_list = list(allow_headers) if isinstance(allow_headers, list) else [str(allow_headers)]
...
async def auto_login_local_user(request: Request, call_next):
    ...
    if is_local and request.url.path != '/login' and not request.url.path.startswith('/auth/'):
...
app.include_router(init_news_router(chat_model))
app.include_router(init_version_router())
app.include_router(init_messenger_router())
app.include_router(init_helpdesk_router())
app.include_router(init_network_router())
app.include_router(init_system_router(chat_model))
app.include_router(init_ifttt_router())
app.include_router(init_windows_admin_router())
...
def is_authenticated_user(request: Request) -> bool:
    """Check if the incoming request has a valid authenticated JWT session token."""
...
async def login_page(request: Request):
    """Serving of login & registration page (or redirect to root / if already authenticated)."""
    if is_authenticated_user(request):
...
async def remote_mic_interface() -> HTMLResponse:
    ...
    Returns the main single-page application for voice-driven message dispatching
    and AI chat streaming.

    Returns:
        HTMLResponse: Rendered HTML content of the voice remote control page.

    Exceptions:
        HTTPException: Raised with 500 status code if index.html cannot be read.

    Examples:
        >>> response = await remote_mic_interface()
...
async def remote_mic_static(full_path: str) -> HTMLResponse:
    ...
    Args:
        full_path (str): Relative path to requested static asset within remote_mic directory.

    Returns:
        HTMLResponse: Content of requested static asset.

    Exceptions:
        HTTPException: Raised with 404 status code if target file is not found.

    Examples:
        >>> response = await remote_mic_static('index.html')
        >>> response.status_code
        200
    """
...
async def startup_scheduler():
    """Start the periodic background scheduler on application startup."""
    try:
...
async def shutdown_scheduler():
    """Stop the periodic background scheduler on application shutdown."""
    try:
        from src.utils.scheduler import scheduler
        await scheduler.stop()
    except Exception as ex:
        logger.error(f"Failed to stop background scheduler: {ex}")
