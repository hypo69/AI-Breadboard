...
class Installer:
    ...
    def __init__(self, language: str = "en", install_dir: Optional[Path] = None, light_version: bool = False):
    ...
    def print_header(self):
        ...
        if self.light_version:
            print("Mode: LIGHT VERSION (Gemini / AGY / Cloud Only)")
    ...
    def install_dependencies(self) -> bool:
        ...
        try:
            ...
            # Requirements files
            if self.light_version:
                req_light = self.install_dir / "install" / "req" / "requirements-light.txt"
                req_files = [req_light] if req_light.exists() else [
            ...
            if self.light_version:
                # Ensure google-genai and google-antigravity are explicitly installed
                print("  Installing Google AI providers (gemini, agy)...", end=" ", flush=True)
                subprocess.run(
                    [str(pip_exe), "install", "google-genai", "google-antigravity", "langchain-google-genai"],
                    capture_output=True,
                    text=True
                )
                print("[OK]")
    ...
    def verify_installation(self) -> bool:
        ...
        try:
            ...
            # Patch config.json for light mode
            if self.light_version:
                ...
                if config_path.exists():
                    try:
                        ...
                        print("  Patched config.json for light version [OK]")
                    except Exception as ex:
                        print(f"  [WARN] Failed to patch config.json: {ex}")
    ...
    def download_models(self, skip: bool = False) -> bool:
        ...
        if skip or self.light_version:
            print(self.msg("step_models"))
            print(f"  Skipping local models (light mode)")
            print()
            return True
...
Examples:
  python scripts/cli/installer.py
  python scripts/cli/installer.py --install-light-version
  python scripts/cli/installer.py --lang en
  ...
  python scripts/cli/installer.py --skip-models
    ...
    parser.add_argument(
        ...
        "--light",
        dest="install_light_version",
        action="store_true",
        help="Install lightweight version (Gemini/AGY only, no local heavy AI models/providers)"
    )
    ...
    installer = Installer(
        ...
        light_version=args.install_light_version
    )
    
    return installer.run(
        skip_models=args.skip_models or args.install_light_version,
