...
### 12. **rag_cleaner** — Очистка и санитизация файлов для RAG 🧹
**Категория:** Инструменты / RAG  
**Status:** Активный

#### Назначение
Модульный плагин очистки и семантической подготовки разнородных входных файлов пользователя:
- Извлечение текста из архивов (`.zip`, `.tar`, `.gz`), Word (`.docx`), PDF (`.pdf`), HTML, CSV, JSON, TXT/MD
- Удаление системного мусора (`__MACOSX`, `.DS_Store`, `~$*`)
- Санитизация Unicode (`\uXXXX`), склеивание переносов строк, сжатие пустых строк
- Семантический Markdown чанкинг для векторного индексирования
...
#### Экшены и AI Tools
- `clean_file` — Очистить одиночный файл или архив
- `clean_directory` — Пакетная очистка каталога с экспортом в JSONL
- LLM Tool: `clean_and_chunk_documents_for_rag`

### 13. **plugin_manager** — Управление плагинами ⚙️
**Категория:** Система  
...
**Всего плагинов:** 13 (5 обязательных, 8 опциональных)  
