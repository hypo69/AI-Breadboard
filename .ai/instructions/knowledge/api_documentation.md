...
## 🛟 Helpdesk & Support Center (`/api/helpdesk`, `/helpdesk`)

**Router:** [`src/fastapi/helpdesk/router_helpdesk.py`](file:///C:/Users/onela/AppData/Local/AI-Breadboard/src/fastapi/helpdesk/router_helpdesk.py)

Dedicated support ticket coordination, real-time message stream, and operator dashboard.

### Endpoints

- **`GET /api/helpdesk/tickets`** — Query and filter tickets by status (`open`, `in_progress`, `resolved`, `closed`), priority (`low`, `normal`, `high`, `urgent`), or search term.
- **`POST /api/helpdesk/tickets`** — Create a new support ticket and send initial message.
- **`GET /api/helpdesk/tickets/{ticket_id}`** — Retrieve ticket details and conversation thread.
- **`PATCH /api/helpdesk/tickets/{ticket_id}`** / **`PUT /api/helpdesk/tickets/{ticket_id}`** — Modify ticket status, priority, or assigned operator.
- **`PUT /api/helpdesk/tickets/{ticket_id}/status`** — Direct endpoint for updating ticket lifecycle status.
- **`PUT /api/helpdesk/tickets/{ticket_id}/priority`** — Direct endpoint for modifying ticket priority level.
- **`POST /api/helpdesk/tickets/{ticket_id}/messages`** — Post user reply, operator message, or internal note.
- **`GET /api/helpdesk/stats`** — Aggregate metrics on open, in-progress, resolved, and urgent tickets.
- **`WS /api/helpdesk/ws/{client_id}`** — Real-time bidirectional WebSocket stream for tickets, chat, typing events, and alerts.
