...
В подсистеме агентов (`src/api/router_agents.py`, `src/api/router_mcp.py`) реализован агентный цикл ReAct:
...
Вместо написания уникальных адаптеров для каждого сервиса, `AI-Breadboard` подключается к MCP-серверам через JSON-RPC по протоколам `stdio` или `SSE`:
...
В [`src/tts/`](../../../src/tts/) и маршрутизаторе TTS (`src/api/router_tts.py`) реализован **двухэтапный конвейер генерации**:
...
2. **Синтез лаконичной речи (Narrator Model):** Compression ответа до 1-2 емких предложений для озвучивания голосовым движком (Edge-TTS / gTTS) без задержки восприятия.
...
2. Протокол MCP стандартизирует connection к базам данных, веб-сервисам и файловым системам.
3. Splitting экранного ответа и голосового резюме обеспечивает оптимальный пользовательский опыт в мультимодальных интерфейсах.
