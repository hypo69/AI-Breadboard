...
> **Цель главы:** Изучить методы оптимизации и сжатия моделей (квантование, merging операторов), освоить пайплайн экспорта весов в ONNX, а также структуру подготовки датасетов и дообучения малых языковых моделей (SLM) через LoRA / QLoRA.
...
В [`src/ai/providers/onnx/`](../../../src/ai/providers/onnx/) реализован инструмент конвертации весов Hugging Face и GGUF в формат ONNX через библиотеку `optimum.onnxruntime`.

### Ключевые возможности конвертера:
1. **Асинхронный экспорт:** Выполняется в фоновом пуле потоков без блокировки FastAPI.
2. **Экспорт графа и весов:** Splitting больших моделей на чанки весов (`model.onnx_data`) для преодоления 2 ГБ лимита формата Protobuf.
...
- **Attention Fusion (Merging внимания):** Объединение матричных умножений Query, Key, Value в один высокоэффективный тензорный оператор.
...
2. Module `gguf_to_onnx.py` автоматизирует transition от PyTorch к кроссплатформенным рантаймам.
