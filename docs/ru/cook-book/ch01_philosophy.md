...
|-------|----------|------------|
| `/api/*` | REST (JSON) | Отправка запросов, управление настройками, Loading файлов |
...
Серверная часть (`main.py`, `src/fastapi/`, `src/api/`) принимает запросы и performs первичную обработку:
- Authentication через JWT
- Validation входных данных (Pydantic)
...
Ядро системы — `src/ai/model_manager.py` и `src/ai/unified_chat.py` — определяет, какой провайдер будет обрабатывать запрос:
...
|---|---|---|---|
| Google Gemini SDK | `gemini-*` | `src/ai/providers/gemini/` | Облако |
| OpenAI-совместимые | `openai:`, `deepseek:`, `lmstudio:` | `src/ai/providers/openai/` | Облако / локально |
| Microsoft AI Foundry | `foundry:` | `src/ai/providers/foundry/` | Локальный сервер |
| ONNX Runtime | `onnx:` | `src/ai/providers/onnx/` | Локальный GPU (DirectML) |
| Hugging Face | `hf:` | `src/ai/providers/huggingface/` | Локальный (PyTorch) |
| Ollama | `ollama:` | `src/ai/providers/ollama/` | Локальный демон |
| Gemini CLI | `gemini_cli:` | `src/ai/providers/gemini_cli/` | Локальный CLI-агент |
| Antigravity (AGY) | `agy:` | `src/ai/providers/agy/` | Облако / AGY SDK |
| Windows AI | `windows_ai:` | `src/ai/providers/windows_ai/` | NPU / DirectML Native |
...
**Ключевой принцип:** пользователь не задумывается о том, какая модель processes запрос — достаточно выбрать нужную из выпадающего списка, а оркестратор сам направит запрос соответствующему провайдеру.
...
```
AI-Breadboard/
├── src/                        # Архитектура и ядро базовой платы
│   ├── ai/                     # Сокеты для чипов и адаптеры моделей
│   │   ├── providers/          # Модульные пакеты провайдеров (gemini, ollama, onnx, etc.)
│   │   ├── model_manager.py    # Контроллер сокетов и Circuit Breaker
│   │   ├── unified_chat.py     # Главная сигнальная шина
│   │   └── base_provider.py    # Базовый контракт адаптеров
│   ├── api/                    # REST API эндпоинты и маршрутизация
│   ├── fastapi/                # Веб-панель управления и статика
│   ├── rag/                    # Векторные эмбеддинги и RAG-память
│   ├── logger/                 # Диагностика сигналов и логирование
│   ├── secrets/                # Регулирование питания и пул API-ключей
│   ├── skills/                 # Реестр и диспетчеризация навыков
│   ├── plugins/                # Реестр системных плагинов
│   └── utils/                  # Утилиты и сериализация данных
├── .agents/skills/             # 25 расширяемых навыков моделей
├── plugins/                    # 10 системных и интеграционных плагинов
├── launchers/                  # Скрипты запуска сервисов и демонов
...
├── config.json                 # Настройки платы и Configuration сокетов
└── run.ps1                     # Главный выключатель питания стенда
```

---

## 1.4. Configuration сокетов и splitting секретов

Как и в электрической схеме, Parameters распиновки и чувствительные ключи питания строго разделены:

### 1. `config.json` — Публичные Parameters и распиновка
Parameters работы сервера, порты сокетов, таймауты и черные списки отключенных чипов:
...
| Скрипт | Назначение |
|---|---|
| [`run.ps1`](file:///c:/Users/onela/AppData/Local/aibreadboard/run.ps1) | Главный запуск стенда: check портов, старт локальных демонов и запуск FastAPI с живой перезагрузкой. |
...
2. Четкое splitting настроек в `config.json` и секретов в `.env` обеспечивает безопасность и удобство конфигурирования.
3. Moduleные лаунчеры дают возможность включать только те блоки стенда, которые необходимы для конкретного эксперимента.
