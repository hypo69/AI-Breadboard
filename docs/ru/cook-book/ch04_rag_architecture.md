...
> **Цель главы:** Понять математику семантических эмбеддингов, устройство легковесных векторных индексов FAISS без внешних СУБД, алгоритмы гибридного поиска и splitting задач между локальным сервером и Google Colab.
...
Основой RAG является conversion текста в многомерный вектор (эмбеддинг):
...
## 4.2. Пороги релевантности и filtering шума

В модуле [`src/rag/`](../../../src/rag/) и логике диспетчеризации принята строгая градация порогов сходства:
...
В [`src/rag/`](../../../src/rag/) применен минималистичный и надежный подход:
...
## 4.4. Индексатор кодовой базы: `rebuild_dev_rag.py`

В скрипте [`scripts/maintenance/rebuild_dev_rag.py`](../../../scripts/maintenance/rebuild_dev_rag.py) реализован механизм семантического поиска по самой кодовой базе `AI-Breadboard`.

Функция сканирует директории `src/`, `docs/`, `prompts/`, извлекает структуру файлов `.py` и `.md` и строит локальный векторный индекс. Это позволяет ИИ-агентам мгновенно находить релевантные функции и документацию по естественному описанию.
...
1. Loading SQLite базы или текстового корпуса на Google Drive.
