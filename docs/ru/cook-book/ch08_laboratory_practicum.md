...
- **Задание:** Запустить `./run.ps1`, открыть Swagger документацию `http://localhost:8000/docs` и отправить тестовый ping-запрос к API.
- **Критерий успеха:** Сервер Returns status `200 OK`, в логах `logs/` отсутствуют ошибки инициализации.

---

### 🧪 Лабораторная работа 2. Connection нового провайдера в `UnifiedChatModel`
- **Цель:** Освоить расширение маршрутизатора моделей.
- **Задание:** В файле [`src/ai/unified_chat.py`](../../../src/ai/unified_chat.py) изучить префиксную диспетчеризацию и добавить поддержку нового префикса `custom:<model_name>`.
- **Критерий успеха:** При передаче запроса с моделью `custom:test` управление передается в ваш обработчик.

---

### 🧪 Лабораторная работа 3. Check отказоустойчивости (Circuit Breaker)
...
- **Задание:** Загрузить квантованную модель в [`src/ai/providers/onnx/`](../../../src/ai/providers/onnx/) с провайдером `DirectMLExecutionProvider` и измерить время генерации 50 токенов.
- **Критерий успеха:** Loading GPU отображается в диспетчере задач Windows без падения процесса.

---

### 🧪 Лабораторная работа 5. Построение семантического индекса кодовой базы
- **Цель:** Создать RAG-индекс по собственному репозиторию.
- **Задание:** Запустить скрипт `scripts/maintenance/rebuild_dev_rag.py` и выполнить поиск функции роутинга запросом *«Как работает переключение моделей»*.
- **Критерий успеха:** Function Returns релевантный фрагмент `unified_chat.py` со score $> 0.70$.
...
- **Цель:** Освоить Module `gguf_to_onnx.py`.
...
- **Критерий успеха:** В выходной директории сформирован valid файл `model.onnx` и токенизатор.
...
- **Критерий успеха:** Агент активирует навык, performs скрипт бенчмаркинга и Returns структурированную сравнительную таблицу.
