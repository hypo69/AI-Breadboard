...
Добавьте раздел в [GEMINI.md](../../../GEMINI.md), чтобы агент при старте всегда видел ссылки на историю решений:
```markdown
...
1. **Автоматическая загрузка:** При открытии проекта на любом компьютере агент первым делом читает [GEMINI.md](../../../GEMINI.md).
