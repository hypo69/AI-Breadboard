...
1. Перейдите в раздел [Конфигурация](secrets.md) и [Справочник `config.json`](config-reference.md)
2. Настройте файл `.env` на основе локального шаблона `.env.example`
