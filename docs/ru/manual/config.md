...
> `config.json` версионируется в Git и содержит **только публичные настройки и параметры архитектуры**. Приватные токены и ключи API хранятся в файле [`.env`](../../../.env.example) и защищенном пуле [`src/secrets/gemini_keys.json`](configuration.md#2-управление-ключами-google-gemini-пул-ротации).
