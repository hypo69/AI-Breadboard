...
2. **Запись в файлы без дублирования**:
   - Логи записываются в файлы `info.log`, `debug.log`, `errors.log` и `log.json` в директории `tmp/logs/`.
...
3. **Сбор логов сторонних сервисов**:
   - При запуске через `run.ps1` логи от процессов `uvicorn` и `foundry` перенаправляются в единую папку `tmp/logs/`:
     - `tmp/logs/uvicorn.log` — логи веб-сервера FastAPI.
     - `tmp/logs/foundry_stdout.log` / `tmp/logs/foundry_stderr.log` — логи локальной службы ИИ Microsoft AI Foundry.

4. **Интеллектуальный анализ логов**:
   - Фоновая служба `log_analyzer` периодически сканирует директорию `tmp/logs/`.
   ...
   - Модель формирует отчет (`tmp/reports/report_<имя_лога>_<дата>.md`) с разбором ошибок, предупреждений, трендов и рекомендаций по исправлению.
   - После успешного анализа оригинальный лог-файл очищается (размер становится 0 байт).
   - Все индивидуальные отчеты автоматически консолидируются в единый журнал состояния системы `tmp/reports/master_journal.md`.
...
- [logger.py](file:///c:/ai-assistant/core/logger/logger.py) — Класс инициализации консольного, файлового и JSON логирования.
- [log_analyzer.py](file:///c:/ai-assistant/core/logger/log_analyzer.py) — Скрипт периодического мониторинга размеров файлов и генерации умных отчетов с Gemini.
- [main.py](file:///c:/ai-assistant/main.py) — Точка входа приложения, запускающая фоновую задачу анализатора при старте FastAPI.
